SRCS_FILES= ft_atoi.c ft_bzero.c ft_calloc.c \
ft_isalnum.c ft_isalpha.c ft_isascii.c \
ft_isdigit.c ft_isprint.c ft_isspace.c \
ft_itoa.c ft_memccpy.c ft_memchr.c \
ft_memcmp.c ft_memcpy.c ft_memmove.c \
ft_memset.c ft_putchar.c ft_putchar_fd.c \
ft_putendl_fd.c ft_putnbr.c ft_putnbr_fd.c \
ft_putstr.c ft_putstr_fd.c ft_split.c \
ft_strchr.c ft_strcmp.c ft_strdup.c \
ft_strjoin.c ft_strlcat.c ft_strlcpy.c \
ft_strlen.c ft_strmapi.c ft_strncmp.c \
ft_strncpy.c ft_strnstr.c ft_strrchr.c \
ft_strtrim.c ft_substr.c ft_tolower.c \
ft_toupper.c

BONUS_SRCS_FILES= ft_lstadd_back.c \
ft_lstadd_front.c ft_lstclear.c \
ft_lstdelone.c ft_lstiter.c \
ft_lstlast.c ft_lstmap.c \
ft_lstnew.c ft_lstsize.c

OBJ = $(SRCS_FILES:.c=.o)
BONUS_OBJS= $(BONUS_SRCS_FILES:.c=.o)

NAME=libft.a
CC=clang
CFLAGS=-Wall -Wextra -Werror -O3 -I.

all: $(NAME)

$(NAME): $(OBJ)
	ar rc $(NAME) $(OBJ)

bonus: $(OBJS) $(BONUS_OBJS)
	ar rcs $(NAME) $(OBJS) $(BONUS_OBJS)

so:
	$(CC) -shared -o libft.so $(CFLAGS) $(BONUS_SRCS_FILES) $(SRCS_FILES) -fPIC

clean:
	rm -f $(OBJ) $(BONUS_OBJS)

fclean: clean
	rm -f $(NAME)

re: fclean all

.PHONY: all bonus re clean fclean